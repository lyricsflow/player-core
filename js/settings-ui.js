...
function _resolveSpring({ stiffness = 170, damping = 26, mass = 1, bounce, duration } = {}) {
  if (duration !== undefined && bounce !== undefined) {
    const zeta = _clamp(1 - bounce, 0.0001, 1);
...
function _springToEasing(config = {}) {
  const { zeta, omega } = _resolveSpring(config);
  const durationSec = config.duration !== undefined ? config.duration : _settleTime(zeta, omega);
  const steps = _clamp(Math.round((durationSec * 1000) / 8), 24, 480);
  const points = [];
...
function _openMenu(menu) {
  ...
  aeroAnimate(menu, [_DROPDOWN_ENTER.hidden, _DROPDOWN_ENTER.shown], { spring: { duration: 0.3, bounce: 0.15 } });
...
function _closeMenu(menu) {
  ...
  aeroAnimate(menu, [_DROPDOWN_ENTER.shown, _DROPDOWN_ENTER.hidden], { spring: { duration: 0.18, bounce: 0 } })
...
function initDropdowns(root = document) {
  root.querySelectorAll('.aero-dropdown').forEach(dropdown => {
    ...
    trigger.addEventListener('keydown', e => { if (e.key === 'ArrowDown') { e.preventDefault(); if (!menu.classList.contains('aero-menu--open')) _openMenu(menu); const items = [...menu.querySelectorAll('.aero-menu-item:not(:disabled)')]; items[0]?.focus(); } });
    menu.addEventListener('keydown', e => {
      const items = [...menu.querySelectorAll('.aero-menu-item:not(:disabled)')];
  ...
  document.addEventListener('click', e => {
    document.querySelectorAll('.aero-menu.aero-menu--open').forEach(menu => {
      if (!menu.closest('.aero-dropdown')?.contains(e.target)) _closeMenu(menu);
    });
  });
  document.addEventListener('keydown', e => {
    ...
    document.querySelectorAll('.aero-menu.aero-menu--open').forEach(menu => {
      _closeMenu(menu);
      menu.closest('.aero-dropdown')?.querySelector('[data-aero-dropdown]')?.focus();
    });
  });
}
